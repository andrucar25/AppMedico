...
class HistoryPacienteTeleconsultation extends StatefulWidget {
  static String routeName = "/history_paciente_teleconsultation";
  HistoryPacienteTeleconsultation({Key key}) : super(key: key);

  @override
  _HistoryPacienteTeleconsultationState createState() =>
      _HistoryPacienteTeleconsultationState();
}

class _HistoryPacienteTeleconsultationState
    extends State<HistoryPacienteTeleconsultation> {
  final provider = TeleconsultaProvider();
  @override
  Widget build(BuildContext context) {
    int idPaciente = ModalRoute.of(context).settings.arguments;
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Column(
      ...
      body: FutureBuilder(
        future: provider.getTeleconsultaHistorialPaciente(idPaciente),
        builder:
            (BuildContext context, AsyncSnapshot<List<Teleconsulta>> snapshot) {
          ...
          if (snapshot.data.isEmpty) {
            return Padding(
              padding: EdgeInsets.only(top: getProportionateScreenHeight(220)),
              child: Center(
                  child: Column(
                children: <Widget>[
                  Stack(
                  ...
                  RichText(
                    ...
                    TextSpan(
                        text:
                            "¡El paciente no tiene teleconsultas finalizadas!",
                        style: TextStyle(
                            color: Colors.black,
          ...
          return GroupedListView<Teleconsulta, String>(
            ...
            groupBy: (item) => getEspecialidadName(item.idEspecialidad),
                                ...
                                children: [
                                  Padding(
                                    padding: EdgeInsets.only(
                                        ...
                                        right: getProportionateScreenWidth(8)),
                                    child: Text(
                                      'Dr. ${item.nombresMedico} ${item.apellidosMedico}',
                                      /*"Dr." +
                                              " " +
                                              item.nombresMedico ?? 'Default' +
                                              " " +
